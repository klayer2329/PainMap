  ...
  Object.entries(conditionSet).forEach(([key, cond]) => {
    ...
    (cond.rules || []).forEach((rule) => {
      if (ruleHits(answers, rule)) {
        total += rule.points;
        factors.push({ points: rule.points, desc: ruleDesc(rule), source: "base" });
      }
    });
    scores[key] = total;
...
// ---------------- Injury History Module ----------------
// 用户选择过往患过的伤病：weight = 0-10，选中的伤病分数 *= (1 + 0.1 * weight)
function applyHistoryMultiplier(scores, historySelections, weight, supporting) {
  const w = Number(weight) || 0;
  (historySelections || []).forEach((key) => {
    if (Object.prototype.hasOwnProperty.call(scores, key)) {
      const before = scores[key];
      scores[key] = scores[key] * (1 + 0.1 * w);
      if (supporting) {
        supporting[key] = supporting[key] || [];
        supporting[key].push({
          points: Math.round((scores[key] - before) * 10) / 10,
          desc: `既往病史加权 ×(1 + 0.1 × ${w})`,
          source: "history",
        });
      }
...
// 旧函数名别名，兼容早期调用
function applyInjuryHistory(scores, historySelections, weight, supporting) {
  return applyHistoryMultiplier(scores, historySelections, weight, supporting);
}
