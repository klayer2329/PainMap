...
function showWelcome() {
  ...
  `);
  document.getElementById("startBtn").onclick = showRedFlagVas;
}
...
function showRedFlagChecklist() {
  ...
  document.getElementById("nextBtn").onclick = () => {
    ...
    if (vasHigh || anyFlag) {
      state.answers.deformity = !!state.answers.deformity; // ensure defined
      showEmergency();
    } else {
      showModeSelect();
    }
...
function showPainMapPrimary() {
  ...
  render(`
    ...
    <p class="subtitle">先选择大致区域，切换视图查看不同角度</p>
    <div class="card">
      <div class="view-tabs" id="viewTabs">
        ${PAIN_VIEW_TABS.map((t) => `<button class="view-tab ${t.id === painMapViewId ? "active" : ""}" data-id="${t.id}">${t.label}</button>`).join("")}
      </div>
      <div class="foot-svg-wrap" id="svgWrap">${renderPainMapSvg(painMapViewId, selected)}</div>
    </div>
    <div class="btn-row">
      <button class="btn btn-secondary" id="backBtn">← 返回</button>
  ...
  document.querySelectorAll("#viewTabs .view-tab").forEach((el) => {
    el.onclick = () => { painMapViewId = el.dataset.id; showPainMapPrimary(); };
  });
  document.querySelectorAll(".foot-region").forEach((el) => {
    el.onclick = () => {
      state.answers.primary_location = el.dataset.region;
...
function showPainMapSecondary() {
  ...
  render(`
    ...
    <div class="card">
      <div class="options">
        ${options.map((o, i) => `<button class="opt ${current === o ? "selected" : ""}" data-v="${o}"><span class="num">${i + 1}</span><span>${o}</span></button>`).join("")}
      </div>
    </div>
    <div class="btn-row"><button class="btn btn-secondary" id="backBtn">← 返回</button></div>
  `);
  ...
  document.querySelectorAll(".opt").forEach((el) => {
    el.onclick = () => { state.answers.secondary_location = el.dataset.v; showPainShape(); };
  });
}
...
function showFunctionalStep() {
  setQuarter(3);
  if (state.fIndex >= FUNCTIONAL_TESTS.length) return showInjuryHistory();
...
// 10. INJURY HISTORY
// ---------------------------------------------------------
function showInjuryHistory() {
  setQuarter(3);
  const conditionSet = state.mode === "acute" ? ACUTE_CONDITIONS : CHRONIC_CONDITIONS;
  const list = Object.entries(conditionSet).filter(([, c]) => !c.hidden);
  const selected = new Set(state.answers.history_selections || []);
  const weight = state.answers.injuryHistoryWeight ?? 3;

  render(`
    <p class="eyebrow">Q4 · 既往伤病史</p>
    <h1 class="title">你之前有没有得过以下伤病？</h1>
    <p class="subtitle">可多选。这会作为背景参考因素，轻微提升相关伤病的排序权重。</p>
    <div class="card">
      <p style="margin:0 0 6px; font-weight:600; font-size:14px;">既往史影响程度：<span id="whVal" style="color:var(--orange);">${weight}</span> / 10</p>
      <input type="range" min="0" max="10" step="1" id="weightSlider" value="${weight}">
      <div class="checklist" style="margin-top:18px;" id="histList">
        ${list.map(([key, c]) => `
          <div class="chk ${selected.has(key) ? "selected" : ""}" data-key="${key}" style="${selected.has(key) ? "border-color:var(--orange); background:#FFF4E9;" : ""}">
            <span class="box" style="${selected.has(key) ? "background:var(--orange); border-color:var(--orange);" : ""}">${selected.has(key) ? "✓" : ""}</span>
            <span>${c.nameZh}</span>
          </div>`).join("")}
      </div>
    </div>
    <div class="btn-row">
      <button class="btn btn-secondary" id="backBtn">← 返回</button>
      <button class="btn btn-primary" id="submitBtn">下一步：特殊检查 →</button>
    </div>
  `);

  document.getElementById("weightSlider").oninput = (e) => {
    document.getElementById("whVal").textContent = e.target.value;
    state.answers.injuryHistoryWeight = Number(e.target.value);
  };
  document.querySelectorAll("#histList .chk").forEach((el) => {
    el.onclick = () => {
      const key = el.dataset.key;
      if (selected.has(key)) selected.delete(key); else selected.add(key);
      state.answers.history_selections = Array.from(selected);
      showInjuryHistory();
    };
  });
  document.getElementById("backBtn").onclick = () => { state.fIndex = FUNCTIONAL_TESTS.length - 1; showFunctionalStep(); };
  document.getElementById("submitBtn").onclick = showScoreBreakdown;
}
...
function showScoreBreakdown() {
  ...
  render(`
    ...
    <p class="subtitle">分数越高，特征越吻合；这里列出每一分具体来自哪道题的哪个选项，方便你核对答案。</p>
...
function computeResults() {
  ...
  applyHistoryMultiplier(scores, state.answers.history_selections, state.answers.injuryHistoryWeight ?? 3, supporting);
